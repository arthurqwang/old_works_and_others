...
if isexp=1 then 
  ...
  response.write "&nbsp;&nbsp;<a href=exitexpert.asp?to=list.asp?boardid=1&catalog="&catalog&"><img src='images/exit.gif' border=0><font size=2 color=#666666>退出</font></a><a href=exppwchg.asp target='_blank'>&nbsp;&nbsp;<img src='images/key.gif' border=0><font size=2 color=#666666>修改注册信息</font></a><a href=othexp.asp target='_blank'>&nbsp;&nbsp;<img src='images/othexp.gif' border=0><font size=2 color=#666666>联系其他专家/嘉宾</font></a>"
end if
%>
</center>
...
select case catalog
	           case "0"
	                response.write "全部BBS文章"
	           case "1"
	                response.write "软件工程与系统集成专题"
	           case "2"
	                response.write "数据与数据库专题"
	           case "3"
	                response.write "网络与基础设施专题"
	           case "4"
	                response.write "GIS专题"
	           case "5"
	                response.write "办公自动化专题"
	           case "6"
	                response.write "多媒体专题"
	           case "7"
	                response.write "系统维护专题"
	           case "8"
	                response.write "虚拟现实专题"
	           case "9"
	                response.write "数据中心专题"
	           case "a"
	                response.write "ERP专题"
	           case "b"
	                response.write "勘探专题"
	           case "c"
	                response.write "开发专题"
	           case "d"
	                response.write "勘探开发一体化专题"
	           case "e"
	                response.write "地面工程专题"
	           case "f"
	                response.write "经营管理专题"
	           case "g"
	                response.write "信息化建设策略与管理专题"
	           case "z"
	                response.write "站务管理与其它专题"
	           case else
	                response.write "全部BBS文章"
end select
...
'   if isexp=1 then
    sql="select AnnounceID,boardID from bbs1 where display=1 and boardID="+cstr(boardID)+" and parentID=0 and status<>4 and (catalog like '%"+catalog+"%') ORDER BY LastReplyID desc "
    if catalog="0" then sql="select AnnounceID,boardID from bbs1 where display=1 and boardID="+cstr(boardID)+" and parentID=0 and status<>4 ORDER BY LastReplyID desc "
'   else
'    sql="select AnnounceID,boardID from bbs1 where display=1 and specialzone=0 and boardID="+cstr(boardID)+" and parentID=0 and status<>4 and (catalog like '%"+catalog+"%') ORDER BY LastReplyID desc "
'    if catalog="0" then sql="select AnnounceID,boardID from bbs1 where display=1 and specialzone=0 and boardID="+cstr(boardID)+" and parentID=0 and status<>4 ORDER BY LastReplyID desc "
'   end if
 ...
 if not  rs.eof then
	 ...
	 if selStr<>"()" then
          ...
          sql="select * from bbs1 where display=1 and (rootID in "&selStr& " ) ORDER BY "+postTmp+"rootID desc,orders"
     else
          ...
          sql="select * from bbs1 where display=1 ORDER BY "+postTmp+"rootID desc,orders"
	 end if
	end if
         rs.Close
...
function showpage(boardtype,boardmaster,totalannounce,boardid,mailstr1,mailstr2)
      dim n
       ...
       response.write "<a href=list.asp?boardID="&boardid&"&amp;page="&(CurrentPage+1)&"&amp;catalog="&catalog&"><font size=2>[下一页]</font></a></td>"
       ...
       response.write "<td nowrap bgcolor=''><p align='left'><a href=list.asp?boardID="&boardid&"&amp;page=1"&"&amp;catalog="&catalog&"><font size=2>[刷新]</font></a></td>"
       ...
       if CurrentPage<2 then
        response.write "<font color='#dddddd' size=2>首页 前页</font>&nbsp;"
          ...
          response.write "<a href=list.asp?boardID="&boardid&"&amp;page=1"&"&amp;catalog="&catalog&"><font size=2>首页</font></a>&nbsp;"
          response.write "<a href=list.asp?boardID="&boardid&"&amp;page="&CurrentPage-1&"&amp;catalog="&catalog&"><font size=2>前页</font></a>&nbsp;"
        end if
        if n-currentpage<1 then
          response.write "<font color='#dddddd' size=2>后页 尾页</font>"
        else
          response.write "<a href=list.asp?boardID="&boardid&"&amp;page="&(CurrentPage+1)&"&amp;catalog="&catalog
          response.write "><font size=2>后页</font></a> <a href=list.asp?boardID="&boardid&"&amp;page="&n&"&amp;catalog="&catalog&"><font size=2>尾页</font></a>"
        end if
        ...
        response.write "<tr><td><font size=2><a href=http://www.digitaloilfield.org.cn target=_blank>论坛首页</a></font></td></tr>"
