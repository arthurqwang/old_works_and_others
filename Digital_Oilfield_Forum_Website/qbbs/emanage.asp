...
.input2 {width: 30px;  height: 16px; font:12px;  border-style: solid; border-width: 1px; border-color: #A5BEE7}
...
.button1 {width: 80px;  height: 17px; font:12px; border-style: plane; background-color: #eeeeee}
-->
</style>
 ...
 if not  rs.eof then
   ...
   sub showlist()
       ...
       do while not (rs.eof or err.number<>0)
          ...
          loop
          tdcl=tdcl+1
          outtext=outtext &  "<li><table border='0' cellpadding='0' cellspacing='0' style='border-collapse: collapse'>"
'          if rs("layer")=1 and rs("child")=0 then outtext=outtext&"<form action='setStatus.asp' method='post' >"
          outtext=outtext&"<tr><td valign='bottom' bgcolor='#"
          if (tdcl mod 2)=0 then
            outtext=outtext&"ffffff'>"
          else
            outtext=outtext&"dddddd'>"
          end if
...
'      if rs("layer")=1 and rs("child")=0 then outtext=outtext & "<input type='hidden' name='action' value='merge'><input type='hidden' name='announceID' value='"&rs("announceid")&"'><input class='button1' type='submit' value='并入一级标题'><input class='input2' type='text' name='ParentID' size='5' value='"&rs("ParentId")&"'>"
      ...
      rs.movenext
          outtext=outtext&"</td></tr>"
'          if rs("layer")=1 and rs("child")=0 then outtext=outtext&"</form>"
