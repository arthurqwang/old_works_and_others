...
/* when MAY_WRITE_TO_H_SEC==2,recover the back hidden sectors */
   ...
   switch(MAY_WRITE_TO_H_SEC)
    {
	 ...
	 if(
		...
		DISKRESET;
		infwq();
		wprintf(inf,"  MAIN BOOT information not found.\n");
		return -1;
	     }
	 else
	     {
     ...
     case 1:  /*日常处理高保护方式:只从后隐扇区中载入备份,若成功,则覆盖前
		隐扇区,若失败,则要求用户运行 MNGRBOOT/B ,日常不从前隐扇区
		载入备份,因其不可靠.*/
   ...
   switch(MAY_WRITE_TO_H_SEC)
    {
	 ...
	 if(
		...
		DISKRESET;
		infwq();
		wprintf(inf,"  %c: DOS BOOT information not found.\n",'C'+8-isector);
		return -1;
	     }
	 else
	     {
   ...
   switch(MAY_WRITE_TO_H_SEC)
    {
	 ...
	 if(
		...
		DISKRESET;
		infwq();
		wprintf(inf,"  Partition information not found.\n");
		return 0;
	     }
	 else
	     {
...
int savemainboot(int drive)
  {
	...
	get_ch();
      ...
      if(MAY_WRITE_TO_H_SEC==0 || MAY_WRITE_TO_H_SEC==1)
