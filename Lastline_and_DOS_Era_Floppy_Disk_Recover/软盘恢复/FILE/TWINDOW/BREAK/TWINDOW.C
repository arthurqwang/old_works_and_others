...
void wprintfr(WINDOW *wnd,char *ln,...)
{	char dlin[100],*dl=dlin;
	if((wnd->_cr) >=(wnd->_ww)-10)
	  wprintf(wnd,"\n");
	if(verify_wnd(&wnd)){
		va_list ap;
		va_start(ap,ln);
...
{	int c=0,ky,d,move_step=1,prvbrdrclr=WCOLOR[BORDER];
	...
	int maxselect=0,have=0,len_keys=strlen(keys),tempp;
	ky=0;
	if(!verify_wnd(&wnd))
		return 0;
	while(line<HEIGHT-1 )
		...
		if(*keys)
		 {
		   ...
		   while(*(keys+ky)){
		    ...
		    {
		     light(wnd,ky+1,hmnnames,cfirst_line,acbg,acfg,acinten);
		     return ky+1;
		    }
		    ky++;
		   }
