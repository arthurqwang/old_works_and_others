...
k3:	     key=getch();
       ...
       if(sig==1)
	 wprintf(inf,"\n  Now,you should remove any disk in drive A: and reboot...\n");
     }
   ...
   if(!strcmp(strupr(argv[1]),"/B"))
     {
       ...
       wprintf(inf,"  Are you sure?[N]\b\b");
k4:    key=getch();
       if(!((toupper(key)=='N')||(toupper(key)=='Y')||(key==13)))
	  goto k4;
       if(toupper(key)=='Y')
    ...
    if (biosdisk(2,drive,0,0,isector,1,boot))
      {
       ...
       wclrprintf(inf,BLUE,WHITE,BRIGHT,"\n  Press any key to retry...");
       getch();
       boot[3]='M';boot[510]=0x55;boot[511]=0xAA;
      }
...
int readparttable(int way) /* way=1 read&backup ,way=0 only read */
  {
    int i=0x209,k,l=1;
    char nm[20]="c:\\aaaaaa.aaa",cmd[40]="mkimg/partn<"  ;
    FILE *fp;
    if(way)
      {
       _chmod("c:\\partinfm.lst",1,FA_RDONLY);
...
void get_BK(void)
 {
   ...
   BKTRACK=((temp[1]&0xff)<<8)+(temp[0]&0xff);
   BKHEAD=temp[2]&0xff;
  }
