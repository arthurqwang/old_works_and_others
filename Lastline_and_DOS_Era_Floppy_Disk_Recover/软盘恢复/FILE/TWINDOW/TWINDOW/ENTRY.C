...
#define FIELDCHAR '_'
 ...
 static void disp_field(WINDOW *wnd,char *bf,char *msk)
 {
  ...
  {
     char *mask=fld->fmask,*buff=fld->fbuff,*bgbuff=buff;
     ...
     default:
	   ...
	   if(toupper(fld->ftype)!='A'&&!isdigit(c)&&c!='.'&&toupper(c)!='E')  {
 ...
 static int validate_date(char *s)
 {
	  ...
	  *(date+2)='\0';
	  ...
	  field_tally(wnd);
	  fld=FHEAD;
  /*-----------collect data entry for a screen template.----------*/
