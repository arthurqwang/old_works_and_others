...
/*LASTLIN2.C  CHINESE VERSION 2.0  1994 3 25*/
...
main(int argc,char *argv[])
 {
   ...
   puthz(icon,"{6822}FAT,ROOT",14,1);
   high(95,high_of_char*10+5,205,high_of_char*10+7,3,1);

   anti_trace();
   check_sec();
   printf("\x7\x7\x7");
   puthz(inf, "              {11450}!           {11450}!\n",15,1);
		/* WARNNING !         WARNNING !*/
   puthz(inf,"{11515} YES ,{11644},{11933} NO {11998}.\n",15,1);
   ...
   for(lgdrive=2;lgdrive<maxdrive;lgdrive++)
   {
     ...
     puthz(inf,"{12191} ",14,1);
     chain[0]=lgdrive+65;
     chain[1]=':';
     chain[2]=0;
     puthz(inf,chain,15,1);
     puthz(inf, "{12288} FAT{12385} ROOT ?[Y]\b\b",14,1);
     ...
     if(biosdisk(2,0x80,0,0,lgdrive+9,1,chain))
         {
           isfail=1;
           DISKRESET;
           puthz(inf,"{12547}FAT{12385}ROOT{12644}.\n",14,1);
           ...
           if((chain[0]!='W')||(chain[1]!='W')||(chain[511]!='Q'))
           ...
           else
              {
                if(biosdisk(3,0x80,0,0,lgdrive+9,1,chain))
                   {
                     DISKRESET;
                     puthz(inf,"{8653}FAT {12385}ROOT {13031}.\n",14,1);
				/*   Error save information for FAT and ROOT.*/
		     puthz(inf,"{7657},{13160}C:\\LASTLINE\\FATRSAVE.\n",14,1);
       ...
       while(((chain[i*2+2]&andnum)*256+(chain[i*2+3]&0xff))<end_chain)
         {
           ...
           if((*rdabs)(lgdrive,SECTOR_PER_CLUSTER,lgsector,fatroot))
             {
               DISKRESET;
               puthz(inf,"{6307}FAT {12385}ROOT{11160}.\n",14,1);
