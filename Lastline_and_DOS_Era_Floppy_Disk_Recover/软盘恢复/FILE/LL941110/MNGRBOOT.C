...
k2:       key =get_ch();
      ...
      if(sig==1)
	{
	 ...
	 puthz(inf,"按<Ctrl+Alt+Del>键热启动...",15,1);
	 printf("\x7");
	 reboot();
	}
      else
...
CONTI:
       ...
       if(sig==1)
	{
	 ...
	 puthz(inf,"  按<Ctrl+Alt+Del>键热启动...",15,1);/*Now,you should reboot,then check virus for hard disk.*/
...
/* MAY_WRITE_TO_H_SEC==2 时,将后隐扇的标记去掉,当修复时只能用前隐扇
   因为修复时总是先载入后隐扇,而 ==2 时不用后隐扇 */
	...
	if(read_del_sec_success)
	  ...
	  {
            puthz(inf,"\n\n\n  现在,请移开A:驱中的盘,需热启动.",14,1);
	    puthz(inf,"\n然后,确保以后的引导顺序总是\"A:,C:\".",14,1);
...
void infwq(void)
 {
   ...
   inf=open_win(14,10,24,70,1,1);
   clr_win(inf,1);
   puthz(inf,"   LASTLINE数据丢失,请立即执行LASTLINE软盘上的命令:\n",14,1);
