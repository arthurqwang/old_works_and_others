...
void set_run_times(void);
void proccess_lh95(void);
int quit(int i);
...
{
  FILE *fp;
  int disk_no,sects,can_install=0,all_0=1,i,key='k';
...
argv[0][0]='a';
  ...
  wclrprintf(titlewnd,WHITE,RED,BLACK,"                组合式多功能管理系统 V95.1\n");
  wclrprintf(titlewnd,WHITE,RED,BLACK,"                         (1995)");
  wclrprintf(titlewnd,WHITE,BLUE,BLACK,"\n                 \n");
  wclrprintf(titlewnd,WHITE,BLUE,BLACK,"                联系电话: (0459)-4892240");
  ...
  if(all_0)
  ...
  else
   {
	...
	can_install=1;
      ...
      {
	wprintf(infownd,"    不能安装,请拨电话:(0459)4892240\n");
          if(anti1==1 ||anti2==1)
	  if(anti2==1 ||anti4==1 && anti5==1)
	quit(0);
	  if(anti5!=1) quit(1);
...
GETCH:
  ...
  if(!(anti2==1 ||anti6==1 ||anti7!=2))  quit(1);
      fclose(fp);
      wprintf(infownd,"\n    正在拷贝文件,请稍候...\n");
      copy_more_Archive_file("a:\\lh95.exe","\\lh\\");
      copy_more_Archive_file("a:\\ucdos\\tch.com","\\lh\\");
      printf("\x7");
      wprintf(infownd,"    请插入第 2 号盘,然后按任意键...");
      get_ch();
      wprintf(infownd,"\n    正在拷贝文件,请稍候...\n");
      copy_more_Archive_file("a:\\*.*","\\lh\\");
      copy_more_Archive_file("\\LH\\LHm.exe","\\lh\\GZ\\lh95.dat");
      proccess_lh95();
      printf("\x7");
      wprintf(infownd,"    请插入第 3 号盘,然后按任意键...");
      get_ch();
      wprintf(infownd,"\n    正在拷贝文件,请稍候...\n");
      copy_more_Archive_file("a:\\*.*","\\lh\\");
      printf("\x7");
      wprintf(infownd,"\n    现在将要初始化数据库.\n");
      wprintf(infownd,"    对于下面的提问,若是初次安装本软件,请回答 Y .\n");
      wprintf(infownd,"    若不是初次安装,请回答 N, 否则用户的数据将丢失.\n");
      wprintf(infownd,"    是否初始化数据库? [Y/N]\b\b\b\b");
GETKEY:
      key=get_ch();
	 ...
	 goto GETKEY;
      ...
      if(toupper(key)=='Y')
      {
      wprintf(infownd,"    请插入第 4 号盘,然后按任意键...");
      get_ch();
      wprintf(infownd,"\n    正在拷贝文件,请稍候...\n");
      copy_more_Archive_file("a:\\*.*","\\lh\\gz\\");
      }
      copy_more_Archive_file("\\LH\\FOXGB.EXE","\\lh\\gz\\");
      remove("foxgb.exe");
      set_run_times();
      system("tch *.*>nul");
      system("tch gz\\*.*>nul");
...
*/
{
   ...
   unsigned char source_drive[3],source_dir[100],source_name[14],source_ext[5];
   ...
   if(dest_name_is_dir)
     ...
     fnmerge(a_dest_file_name,dest_drive,dest_dir,dest_name,dest_ext);
...
void copy_a_file(char *source,char *dest)
{
    ...
    long fl,t;
    int fh,i;
    fh=open(source,O_RDONLY);
    fl=filelength(fh);
    close(fh);
    if((fp1=fopen(source,"rb"))==NULL)
      {
	wprintf(infownd,"    拷贝文件 %s 出错.\n",source);
	return;
      }
      ...
      for(i=0;i<t;i++)
	{
	 ...
	 fwrite(a,4096,1,fp2);
      ...
      fread(a,fl-4096L*t,1,fp1);
      fwrite(a,fl-4096L*t,1,fp2);
/*    while(1)
     {
      fread(&a,1,1,fp1);
      if( feof(fp1)!=0) break;
...
void proccess_lh95(void)
{
  FILE *fp,*fpp;
  int i,j;
  unsigned char a[4096],b[512],c[512];
  long fl,t;
  int fh;
  wprintf(infownd,"    正在处理数据,请稍候...\n");
  fh=open("\\lh\\GZ\\lh95.dat",O_RDONLY);
  fl=filelength(fh);
  close(fh);
  if((fpp=fopen("\\lh\\GZ\\lh95.dat","rb+"))==NULL)
    {
     wprintf(infownd,"    对不起,不能安装.\n");
     quit(0);
    }
  t=fl/4096L;
	...
	{
	 fread(a,4096,1,fpp);
	 fseek(fpp,-4096,SEEK_CUR);
	 for(j=0;j<4096;j++)
	   a[j]=(~a[j])&0xFF;
	 fwrite(a,4096,1,fpp);
	}
      fread(a,fl-4096L*t,1,fpp);
      fseek(fpp,-(fl-4096L*t),SEEK_CUR);
      for(j=0;j<4096;j++)
	   a[j]=(~a[j])&0xFF;
      fwrite(a,fl-4096L*t,1,fpp);
/* while(1)
     {
      fread(&a,1,1,fpp);
      if( feof(fpp)!=0) break;
...
void set_run_times(void)
{
  FILE *fp;
  unsigned char a[512];
  if((fp=fopen("\\lh\\srt.ovl","wb"))==NULL)
    {
     wprintf(infownd,"    对不起,不能安装.\n");
     quit(0);
    }
