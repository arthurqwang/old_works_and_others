...
/*FATRSAVE.C  CHINESE VERSION 2.0  1994 3 25*/
   ...
   if((fp=fopen(filenm,"r+b"))==NULL)
     {
       ...
       puthz(inf,"{15313}FAT{12385}ROOT.\n",14,1);
       exit(1);
     }
    ...
    if(p>51200)
      {
       ...
       hanzi[0]=lgdrive-2+'C';
       hanzi[1]=0;
       puthz(inf,hanzi,14,1);
       puthz(inf,": {1127} FATRSAVE.IMG .\n",14,1);
       exit(0);
      }
    q=fatroot[p+1]&0xff;
    h=fatroot[p]&0xff;
    ...
    for(i=0;i<total_sector;i+=9)
      ...
      if(!p)
		...
		/*	  wprintf(inf,"   No file FAT&BOOT.IMG.\n");*/
          inf=open_win(14,10,24,70,1,1);
	  puthz(inf,"{15539}FAT&ROOT.IMG {15604}.\n",14,1);
          exit(1);
        }
