/*******************************************************************
  ...
  Date:          NOV/10/94
  Author:        WangQuan
  Function:      To install MSS 2.0 Chinese version.
...
/*   if(getenv("MSS")==NULL) {setdisk(olddvr);exit(1);}*/
   LL_exist_flag = (!findfirst("C:\\MSS",&fff,FA_DIREC));
   if(argv[0][1]==':')
     setdisk(toupper(argv[0][0])-'A');
   dvr[0]=toupper(argv[0][0]);
   dvr[2]=0;
   mkdir("c:\\MSS");
   system("copy MSSHZ.LIB c:\\MSS>NUL");
   title("c:\\MSS\\MSSHZ.LIB");
   wrt_SN_with_fd();
...
/************************************************/
  ...
  setone();setone();
  setone();
  /*system("exit");*/system("copy BTMNGR.exe  c:\\MSS>NUL");
  setone();setone();setone();setone();
  setone();setone();
  setone();
  /*system("exit");*/system("copy SAVEFAT.exe  c:\\MSS>NUL"); setone();
  ...
  setone();setone();setone();setone();setone();  setone();
  strcat(temp,tempp);
  strcat(temp," c:\\MSS\\MSSSYS.EX>NUL");
  setcolor(3);
...
void set_l(void)
  {
    ...
    _chmod("c:\\MSS\\MSS.cfg",1,FA_ARCH);
    if((fp=fopen("c:\\MSS\\MSS.cfg","wb+"))==NULL)
       {
	 puthz(inf,"\n错误.\n",0,7);
	 quit(1);
       }
    ...
    fwrite(vol,501,1,fp);
    fclose(fp);
    _chmod("c:\\MSS\\MSS.cfg",1,FA_RDONLY+FA_HIDDEN);
    }
