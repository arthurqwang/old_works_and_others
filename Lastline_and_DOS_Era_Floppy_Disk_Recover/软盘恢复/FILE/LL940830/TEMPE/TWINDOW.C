/*******************************************************************
  ...
  Date:          8/29/94
  Author:        WangQuan
  Function:      To make windows for  LASTLINE 2.5 English version.
...
int print_menu_name(WINDOW *wnd,char **hmnnames,int hmnbg,int hmnfg,
		   int hmninten,char *keys,int kchbg,int kchfg,
		   int kchinten);
...
{	if(!verify_wnd(&wnd))
	...
	switch(c){
		...
		case '\n':
			...
			break;
		case '\t':
			do displ(wnd,(WCURS++)+3,SCROLL+1,' ',clr(bg,fg,
				 inten));
				while((WCURS%TABS)&&(WCURS+1)<WIDTH-1);
			break;
		default:
			if((WCURS+1)<WIDTH-1){
				displ(wnd,WCURS+1,SCROLL+1,c,clr(bg,fg,
				      inten));
				WCURS++;
			}
			break;
		...
		if (cch==toupper(ch))
		   {
		     ...
		     /*because the corner of text area is at(0,0)*/
