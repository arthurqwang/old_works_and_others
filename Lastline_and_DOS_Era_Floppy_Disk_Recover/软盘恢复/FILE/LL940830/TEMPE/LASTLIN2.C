...
main(int argc,char *argv[])
 {
   ...
   unsigned i_total_sector,total_sector,sector_of_nonDOS,end_chain,
	    max_chain;
...
/*   call_at();*/   /* include anti_trace */
   ...
   for(lgdrive=2;lgdrive<maxdrive;lgdrive++)
   {
     ...
     if(isfail)
	{
     ...
     wprintf(inf,
	      "   Restore FAT and ROOT for Logical disk %c:,Sure?[Y]\b\b",
	      lgdrive+65,15);
kkk: key=get_ch();
     printf("\x7");
...
void infwq(void)
 {
   ...
   flag=1;
   printf("\x7\x7\x7");
   wprintf(inf,"\n   LASTLINE data is lost.Run FATRSAVE in C:\\LASTLINE");
   wclrprintf(inf,BLUE,WHITE,BRIGHT,"\n  Press any key to continue...");
   get_ch();
   printf("\x7");
   }
