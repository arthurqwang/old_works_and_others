...
k4:    key=get_ch();
  ...
  /*system("exit");*/
  system("copy fatrsave.exe  c:\\lastline>c:\\aaaaaa.ll$");
  wputchar(p,219);
  wputchar(p,219);wputchar(p,219);
  wputchar(p,219);wputchar(p,219);wputchar(p,219);
  wputchar(p,219);wputchar(p,219);
...
void check_sec(void)
 {
   ...
   if(absread(toupper(dvr[0])-65,1,719/*2000*/,part))
   /*High density(1.2M) uses 2000*/
    if(absread(toupper(dvr[0])-65,1,719,part))
    /*Low density(360K) uses 719*/
     if(absread(toupper(dvr[0])-65,1,719,part))
     {
       biosdisk(0,toupper(dvr[0])-65,0,0,0,0,0);
       wprintf(inf,"\n  Error.\n");
...
"\n   Special hard disk.\n   May only be installed in Low-Protected-Mode.");
  ...
  for(i=0;i<512*total_test_sec_num;i++)
	...
	if(
	  ...
	  {
	      wprintf(inf,
	      "\n     Abmormal hard disk status.Something may go wrong");
	      wprintf(inf,
	      "\n   if in High_Protected_Mode,see User's Manual.");
	      wprintf(inf,
