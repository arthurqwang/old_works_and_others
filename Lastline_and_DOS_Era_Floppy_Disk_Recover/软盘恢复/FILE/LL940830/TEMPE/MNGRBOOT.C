...
#define THISDISKEXIST (((int)part[i]&0xff)!=0)||(((int)part[i+1]&0xff)!=0)
		      ||(((int)part[i+2]&0xff)!=0)
...
k1:   key=get_ch();
	...
	{
	  wprintf(inf,
"  Restore DOS BOOT RECORD for logical disk %c:?[Y]\b\b", 75-isector);
k2:	  key =get_ch();
          printf("\x7");
	  if(!((toupper(key)=='N')||(toupper(key)=='Y')||(key==13)))
	     goto k2;
	  ...
	  if(memcmp(tboot,boot,BPS))
	   {
	     printf("\x7\x7\x7");
	     wprintf(inf,
"  Logical disk %c:     DOS BOOT RECORD has been changed!\n", 75-isector);
	     wprintf(inf,"  Restore with original copy? [Y]\b\b");
k3:	     key=get_ch();
             printf("\x7");
	     if(!((toupper(key)=='N')||(toupper(key)=='Y')||(key==13)))
	   ...
	   wprintf(inf,
	   "  Logical disk %c:     DOS BOOT RECORD OK.\n", 75-isector);
CONTI:	   isector--;i+=3;
	 }
       if(sig==1)
	{
    ...
    if (biosdisk(2,drive,0,0,isector,1,boot))
      {
       ...
       DISKRESET;
       wprintf(inf,
		"  Error read the copy of DOS BOOT RECORD from logical
		disk %c:.\n",75-isector);
       wclrprintf(inf,BLUE,WHITE,BRIGHT,"\n  Press any key to retry...");
       printf("\x7");
       get_ch();
...
int loaddosbootagain(int drive,int isector)
  {
	...
	return -1;
       ...
       if(biosdisk(3,drive,0,0,isector,1,boot))
	 {
	   DISKRESET;
	   wprintf(inf,
       "  Error save DOS BOOT RECORD for logical disk %c:.\n",75-isector);
