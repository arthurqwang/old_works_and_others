...
void draw_box(int row,int col,int erow,int ecol,int colo)
   ...
   line((col-1)*8+dd+1,erow*high_of_char-dd+1,ecol*8-dd,
   erow*high_of_char-dd+1);
   line(ecol*8-dd+1,(row-1)*high_of_char+dd+1,ecol*8-dd+1,
   erow*high_of_char-dd+1);
   high((col-1)*8+5,(row-1)*high_of_char+5,ecol*8-5,
   erow*high_of_char-5,colo,1);
   nohigh((col-1)*8+7,(row-1)*high_of_char+7,ecol*8-7,
   erow*high_of_char-7,colo,2);
   }
	...
	if((win->curline)>=(win->erow))
	 {
	   ...
	   {
	    getimage((win->col)*8+8,ii*high_of_char-4,
	    (win->ecol)*8-16,ii*high_of_char+19,tmp124);
	    putimage((win->col)*8+8,(ii-2)*high_of_char-4,tmp124,0);
	   }
	  setcolor(bkcolor);
	  bar((win->col)*8+10,(ii-2)*high_of_char-2,
	  (win->ecol)*8-18,(ii-2)*high_of_char+16);
	...
	while((i=*hz)!=0){
		    ...
		    if(*hz=='{')
		       ...
		       while((*hz)!='}')
			 {
		       ...
		       by[j]=0;
		       offset=atol(by);
		       x=write_chinese_sentence(win,
		       CCLIB,offset,x,y,color,bkcolor);
		      }
		    else{
			  ...
			  if((win->curline)>=(win->erow))
			    {
				  for(ii=(win->row)+3;
				   ii<(win->erow)-1;ii+=2)
				    {
				      getimage((win->col)*8+8,
				      ii*high_of_char-4,(win->ecol)*8-16,
				      ii*high_of_char+19,tmp124);
				      putimage((win->col)*8+8,
				      (ii-2)*high_of_char-4,tmp124,0);
				    }
				  setcolor(bkcolor);
				  bar((win->col)*8+10,(ii-2)*high_of_char-2,
				  (win->ecol)*8-18,(ii-2)*high_of_char+16);
	...
	if((win->curline)>=(win->erow))
	 {
	   ...
	   {
	    getimage((win->col)*8+8,
	    ii*high_of_char-4,(win->ecol)*8-16,ii*high_of_char+19,tmp124);
	    putimage((win->col)*8+8,(ii-2)*high_of_char-4,tmp124,0);
	   }
	  setcolor(bkcolor);
	  bar((win->col)*8+10,
	  (ii-2)*high_of_char-2,(win->ecol)*8-18,(ii-2)*high_of_char+16);
	  win->curline=win->erow- win->detarow;
	}
	x=((win->curcol)+2)*8;y=((win->curline)-1)*high_of_char;
	f=0;
...
void clr_win(WINGRA *win,int hg)
  {
    ...
    if(hg)
     for(i=(win->row)+1;i<(win->erow)-1;i+=2)
       high((win->col)*8+8,i*high_of_char-4,(win->ecol)*8-16,
       i*high_of_char+19,(win->color),1);
...
/*    int graphdrive=EGA,mode=EGAHI,i,j;*/
    short dlboy1[]={   141,50,137,46,135,46,135,44,137,42,137,36,
    139,32,141,30,143,28,147,24,151,22,153,20,155,20,155,16,
    157,20,161,24,167,30,171,36,173,42,173,44,177,46,177,50,173,
    50,169,52,163,54,163,58,173,58,173,60,179,60,179,58,
    167,54,157,54,147,52,151,50,149,48,151,46,149,46,141,50,139,
    54,141,56,149,52,137,60,143,66,135,62,133,60,131,60,129,62,
    131,64,141,68,143,70,145,72,151,74,155,74,161,72,
    167,68,167,66,171,64,167,58,171,64,163,68,165,70,163,68,155,
    70,149,70,147,64,143,66,143,70,0,0};

    if((CCLIB=open(path_of_HZK,O_RDONLY|O_BINARY))==-1)
       {
       printf("Can't find file LSTH.INF or not running in floppy disk.\n");
       exit(0);
       }
    ...
    puthz(win,"       {0} ",15,1);  /*{129} {162} {195}*/
    high(180,high_of_char+9,500,high_of_char*3+5,1,1);
    settextstyle(1,0,2);
    setcolor(0);
    outtextxy(280,high_of_char*2-1,"LASTLINE 2.5");
    setcolor(14);
    outtextxy(277,high_of_char*2-4,"LASTLINE 2.5");
    puthz(win,"\n     {356}:",14,1);
    puthz(win,"{485} ",15,1);
    puthz(win,"{806}:",14,1);
    puthz(win,"{871}",15,1);
...
/************** DLBOY *****************************/
  ...
  high(418,high_of_char+11,494,high_of_char*2+6,1,1);
  settextstyle(0,0,1);
  setcolor(14);
  if(high_of_char==16)
    {
     outtextxy(420,high_of_char*2-3," c 1994.9");
     circle(431,high_of_char*2+1,5);
    }
  else
    {
     outtextxy(420,high_of_char*2-2," c 1994.9");
     circle(431,high_of_char*2+2,6);
    }
...
int write_chinese_sentence(WINGRA *win,int fp,long unsigned off,
			     int x,int y,int fcolor,int bcolor)
{
...
void wrt_SN_with_hd(void)
{
  ...
  settextstyle(0,0,1);
  setcolor(14);
  outtextxy(420,high_of_char*2+10,"SN:");
  outtextxy(445,high_of_char*2+10,a);
  high(418,high_of_char*2+8,494,high_of_char*3+3,1,1);
}
