/******************************************************************
  ...
  Usage:
		 BACKUP:
		    X:\ANYPATH>Y:\ANYPATH\BTMNGR/b<CR>
		 COMPARE:
		    X:\ANYPATH>Y:\ANYPATH\BTMNGR/c<CR>
		 REPARE:
		    X:\ANYPATH>Z:BTMNGR<CR>
		 HELP:
		    X:\ANYPATH>Y:\ANYPATH\BTMNGR/?<CR>

		    X:,Y: = A:,B:,C:,D:...  Z: = A:,B:.
		    BUT C:\MSS\MSS.CFG must be OK.
		    Must in floppy disk when reparing.
  Where stored:  Floppy disk "MSS 2.0 Chinese version
		 Source files"(MSS 2.0 Yingwenban Yuanwenjian)
  Path stored:   \
...
void main(int argc,char *argv[])
 {
   ...
   if(argc==1)  /* when reparing */
    strcpy(path_of_HZK,"\\MSSHZ.LIB");
   else
    strcpy(path_of_HZK,"c:\\MSS\\MSSHZ.LIB");
...
/* MAY_WRITE_TO_H_SEC==2 时,将后隐扇的标记去掉,当修复时只能用前隐扇
   ...
   if(!strcmp(argv[1],"/?"))
     {
       if(check_lock(drive)==0) quit(1);
       puthz(icon,"BTMNGR, 引导记录管理程序",0,7);
       high(95,high_of_char*10+5,317,high_of_char*10+7,3,1);
       puthz(inf,"用法:\n",1,7);
       puthz(inf," BTMNGR    修复引导记录",0,7);
       puthz(inf,"     BTMNGR/C  检查引导记录\n",0,7);
       puthz(inf," BTMNGR/B  备份引导记录    ",0,7);
       puthz(inf," BTMNGR/?  帮助",0,7);
       quit(0);
     }
   close(CCLIB);
   cleardevice();
   ...
   switch(MAY_WRITE_TO_H_SEC)
    {
     ...
     case 1:  /*日常处理高保护方式:只从后隐扇区中载入备份,若成功,则覆盖前
		隐扇区,若失败,则要求用户运行 BTMNGR/B ,日常不从前隐扇区
		载入备份,因其不可靠.*/
...
unsigned check_lock(int drive)
  {
    ...
    FILE *fp;
    char end_inf[20];
    if((fp=fopen("c:\\MSS\\MSS.cfg","rb"))==NULL)
      {
	puthz(inf,"\n非安装使用.",0,7);
	quit(1);
      }
