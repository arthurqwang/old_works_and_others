/*******************************************************************
  ...
  Usage:         X:\ANYPATH>Y:\ANYPATH\SAVEFAT<CR>
		   X:,Y: = A:,B:,C:,D:...
		 BUT C:\MSS\MSS.CFG must be right.
...
unsigned char end_inf[20],BPB[0x20],boot[BPS],fatroot[25600],dvr[10];
union REGS in,out;
struct SREGS dsbx;
struct fatinfo p;
int (*rdabs)();
...
int use_16_bit(unsigned start_cluster,unsigned lgdrive)
  ...
  long pfat;
  unsigned ii=2,temp;
  unsigned char fname[]="C:\\FATIMAGE.CUR";
  FILE *fp;
  fname[0]=lgdrive+'A';
...
unsigned get_start_cluster(unsigned total_sector,unsigned lgdrive)
  {
	...
	if((p=findstr(fatroot,"FATIMAGECUR",BPS*10))!=NULL)
	   break;
      }
      if(!p)
	{
...
void infwq(void)
 {
   ...
   if(inf==NULL)
     inf=open_win(14,10,24,70,7,1);
   puthz(inf,"\n   MSS数据丢失,请立即执行MSS软盘上的命令:\n",0,7);
   puthz(inf,"          BTMNGR/B\n",0,7);
   puthz(inf,"          SAVEFAT",0,7);
   puthz(inf,"\n按任意键继续...",1,7);
   get_ch();
   printf("\x7");
   }
